import { Link, useNavigate } from 'react-router-dom'
import { Hammer } from 'lucide-react'

// SVG brand icons
const TwitterIcon = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z" />
  </svg>
)

const LinkedinIcon = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
    <rect width="4" height="12" x="2" y="9" />
    <circle cx="4" cy="4" r="2" />
  </svg>
)

const GithubIcon = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
    <path d="M9 18c-4.51 2-5-2-7-2" />
  </svg>
)

export default function TechFooter() {
  const navigate = useNavigate()
  
  const handleConstructionSwitch = () => {
    sessionStorage.setItem('portal_choice', 'construction')
    navigate('/construction')
  }

  return (
    <footer className="tech-footer">
      <div className="container">
        <div className="tech-footer-inner">
          <div className="tech-footer-brand">
            <Link to="/technology" className="tech-footer-logo">
              <img src="/tech/logo.png" alt="First Minds Logo" style={{ height: '46px', width: 'auto', objectFit: 'contain' }} />
              <span>First Minds</span>
            </Link>
            <p className="tech-footer-tagline">
              AI-native intelligence for modern teams. Build dashboards, reports, and insights in seconds.
            </p>
            <div className="tech-social-row">
              <a href="#" className="tech-social-btn" aria-label="Twitter">
                <TwitterIcon />
              </a>
              <a href="#" className="tech-social-btn" aria-label="LinkedIn">
                <LinkedinIcon />
              </a>
              <a href="#" className="tech-social-btn" aria-label="GitHub">
                <GithubIcon />
              </a>
            </div>
          </div>

          <div className="tech-footer-col">
            <h4>Company</h4>
            <ul>
              <li><a href="#">About</a></li>
              <li><a href="#">Careers</a></li>
              <li><a href="#">Press</a></li>
              <li>
                <Link to="/construction" onClick={handleConstructionSwitch} className="text-warning d-flex align-items-center gap-1">
                  <Hammer size={12} /> Construction
                </Link>
              </li>
            </ul>
          </div>

          <div className="tech-footer-col">
            <h4>Features</h4>
            <ul>
              <li><a href="#">Dashboards</a></li>
              <li><a href="#">AI Chat</a></li>
              <li><a href="#">Data Warehouse</a></li>
              <li><a href="#">Embedding</a></li>
            </ul>
          </div>

          <div className="tech-footer-col">
            <h4>Resources</h4>
            <ul>
              <li><a href="#">Blog</a></li>
              <li><a href="#">Docs</a></li>
              <li><a href="#">Case Studies</a></li>
              <li><a href="#">Changelog</a></li>
            </ul>
          </div>
        </div>

        <div className="tech-footer-bottom">
          <span className="tech-footer-copy">© {new Date().getFullYear()} First Minds. All rights reserved.</span>
          <div className="tech-footer-legal">
            <a href="#">Privacy</a>
            <a href="#">Terms</a>
            <a href="#">DPA</a>
          </div>
        </div>
      </div>
    </footer>
  )
}
